...
class Place(Screen):
    ...
    def on_enter(self):
        ...
        bl = BoxLayout(orientation='horizontal', size_hint=(.9, .07), pos_hint={'x': .05, 'y': .15})
        bl.add_widget(Button(
            on_press=self.minus,
            background_normal="Data\Image\\pointer.png"\
        ))
        ...
        layout.bind(minimum_width=layout.setter('width'))
        layout.add_widget(Label(
            text=place_inf, size_hint_x=None,
            font_size=self.height / 55
        ))
        al = FloatLayout(size_hint=(1, .17), )
        sv = ScrollView(size_hint=(None, 1), size=(Window.width, Window.height),pos_hint={'x': 0, 'y': .8})
        sv.add_widget(layout)
        al.add_widget(sv)
        # fl.add_widget(Label(
        #     text=place_inf,
